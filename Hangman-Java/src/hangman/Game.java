...
public class Game {
	...
	private void findLetterAndPrintIt() {
		...
		do { //do while loop started
			do { //do while loop inside the do loop
			...
			} while (!letter.matches("[a-z]")); //end of do while loop which is inside another do loop
			...
			if (counter == 0) {//if counter started
				++mistakes;
				{
					System.out.printf(
							"Sorry! There are no unrevealed letters \'%s\'. \n",//printout of unrevealed word
							letter);
				}
			} else {
				System.out.printf("Good job! You revealed %d letter(s).\n",//print out of revealed word with congratulations.
						counter);
			}

		} while (!dashBuff.toString().equals(guesWord)); //end of main do while loop
